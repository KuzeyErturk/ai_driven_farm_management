...
\centering
\vspace*{2cm}
{\LARGE\bfseries AI-Driven Crop Yield Prediction System:\\
Cross-Country Transfer Learning for UK Agriculture\par}
\vspace{1.5cm}
...
Agricultural productivity faces increasing challenges from climate variability and environmental uncertainty. The growing global population, projected to reach 9.7 billion by 2050 \citep{un2019}, and rising food demand require efficient farming practices to manage limited natural resources sustainably. Climate change has intensified extreme weather events such as droughts, floods, and unseasonal temperatures, creating greater uncertainty in crop production cycles \citep{ipcc2021}.

In the UK, agriculture contributes approximately \pounds10 billion annually to the economy, with cereals covering over 3 million hectares of farmland \citep{defra2023}. Wheat alone accounts for approximately 1.8 million hectares, producing around 14 million tonnes annually \citep{ahdb2023a}. Weather variability significantly impacts yields---the wet summer of 2012 caused wheat yields to drop nearly 19\% below average, from 7.8~\tha{} to 6.3~\tha{} \citep{defra2012}, demonstrating how vulnerable UK agriculture is to climatic extremes.
...
\begin{enumerate}
    ...
    \item \textbf{RQ2:} Can cross-country data pooling from climatically similar European countries improve UK crop yield predictions, and under what conditions does transfer succeed or fail?
...
\begin{enumerate}
    ...
    \item Engineer 47~weather features capturing seasonal patterns aligned with crop growth stages.
    \item Develop and compare machine learning models including Ridge regression, Random Forest, and SVR for five UK crops.
    \item Implement a cross-country data pooling pipeline spanning seven European countries.
...
This project focuses on five major UK crops: wheat, winter barley, spring barley, oats, and oilseed rape (OSR). The UK analysis covers the period 2004--2024 using regional yield data from DEFRA \citep{defra2024} and weather data from the UK Met Office \citep{metoffice2024}. Cross-country data spans 2004--2018 (the common overlap period) across seven countries: UK, France, Germany, Ireland, the Netherlands, Belgium, and Denmark. The prediction models use weather-based features only, excluding economic factors (e.g., fertiliser prices) and detailed soil data due to data availability constraints.
...
Crop yield prediction has progressed from simple agronomic rules and statistical regression to sophisticated machine learning approaches. \citet{vanklompenburg2020} conducted a systematic review of 50~studies using ML for crop yield prediction, identifying Random Forest, neural networks, and support vector machines as the most commonly used algorithms. They found that most studies report improvements over traditional regression baselines, but noted that many studies use large-scale national or continental datasets---a luxury not available for UK regional prediction.

\citet{paudel2022} evaluated ML methods (Ridge, KNN, SVR, GBDT) for crop yield forecasting across European countries, achieving a median normalised RMSE of 16.57\%. Their approach pooled subnational regions within each country---the same rationale underlying our cross-country pooling, but applied at a different geographic scale. Notably, they acknowledged that model performance degrades for extreme weather years, a challenge we address through our anomaly-based approach.
...
\citet{moore2015} conducted the first formal attribution of long-term yield trends to climate change in European agriculture, using panel data across 349~subnational regions in 11~countries including the UK. They detected a statistically significant ``climate fingerprint'' on all four crops studied: wheat yields were 2.5\% lower and barley 3.8\% lower than they would have been without post-1989 warming. Their use of detrended yield and weather data with country fixed effects is directly analogous to our detrending and country-indicator approaches.
...
A critical challenge in regional crop yield prediction is data scarcity. \citet{corcoran2023} explicitly identified data bottlenecks for UK crop yield analysis, noting that non-wheat crops are especially data-poor. This directly motivates our cross-country pooling approach as a data augmentation strategy.

The scientific justification for pooling data across European countries rests on the concept of climate analogues. \citet{pugh2016} used climate analogue matching to assess future crop productivity, finding that suitable growing areas shift geographically under climate change rather than disappearing entirely. We extend this concept from descriptive analysis to predictive data augmentation.
...
The concept of training on one domain and predicting in another is well-established in machine learning as transfer learning \citep{pan2010}. In agricultural contexts, this manifests as training on data-rich countries and predicting in data-poor ones. However, direct transfer faces fundamental challenges: different cultivars, soil types, farming practices, and climate baselines between countries mean that a model trained on absolute yields from one country will systematically mispredict another.

\citet{dinh2022} addressed the methodological challenge of model validation with small datasets, proposing Leave-Two-Out cross-validation as a more robust alternative to Leave-One-Out for sample sizes below 100. Their work validates our use of LOOCV while cautioning about its tendency to favour model complexity---a concern we address through Ridge regularisation.

\citet{evans2025} provided a critical perspective on space-for-time substitution (SFTS)---the assumption that spatial variation in climate can substitute for temporal variation. They warned that SFTS can be ``misleading in magnitude and direction'' due to acclimation effects at different timescales. Our cross-country pooling is fundamentally a form of SFTS, and our negative direct transfer results are consistent with their concerns. However, our anomaly-based approach mitigates this by transferring response \emph{shapes} rather than absolute predictions.
...
\begin{enumerate}
    \item \textbf{UK-specific data scarcity:} Most European crop yield studies use countries with hundreds of subnational regions (France: 96~departments, Germany: 16~states). The UK's four regions create a data scarcity challenge that has not been specifically addressed through cross-country augmentation.
    \item \textbf{Cross-country pooling methodology:} While subnational pooling within countries is common \citep{paudel2022}, cross-country pooling for crop yield prediction is not well-explored, and the conditions under which it succeeds or fails are poorly characterised.
    \item \textbf{Anomaly-based transfer:} The combination of yield detrending with weather z-score standardisation for enabling cross-country data pooling has not been systematically evaluated as a data augmentation strategy.
    \item \textbf{Adaptive source selection:} Whether dynamically selecting source countries based on weather scenario similarity improves upon static pooling has not been investigated.
\end{enumerate}
...
This chapter describes the data sources, feature engineering pipeline, and experimental methodology. The work progressed through three phases: (1)~UK-only baseline models, (2)~cross-country data collection and initial pooling experiments, and (3)~the detrended anomaly transfer approach.
...
\begin{itemize}
    \item \textbf{France:} Department-level yields (1995--2018) from the Schauberger dataset \citep{schauberger2018}, aggregated to four macro-regions (Nord, Ouest, Est, Sud) using area-weighted averaging. This dataset provides longer historical depth than Eurostat.
...
Weather data for all European countries was obtained from the E-OBS gridded dataset (version~31/32) \citep{cornes2018}, which provides daily temperature, precipitation, and radiation on a 0.1\degrees{} grid across Europe. The processing pipeline involved:

\begin{enumerate}
    \item Loading NetCDF files for daily Tmax, Tmin, precipitation, and radiation
...
For France, four macro-regions were defined by aggregating departments. For Germany, four regions (Nord, West, Ost, S\"{u}d) were created. Ireland, the Netherlands, Belgium, and Denmark were treated as single national regions due to their smaller geographic size.

\begin{table}[H]
\centering
\caption{Cross-country data summary. Common overlap period: 2004--2018.}
...
Raw monthly weather data was transformed into 47~seasonal features aligned with crop growth stages. This is critical because crops respond to weather during specific phenological windows, not uniformly throughout the year \citep{zhao2019}.

\subsection{Seasonal Framework}

Weather variables were aggregated into four seasons matching UK crop growth cycles:

\begin{table}[H]
\centering
\caption{Seasonal definitions and crop growth stage relevance}
\label{tab:seasons}
\begin{tabular}{lll}
\toprule
\textbf{Season} & \textbf{Months} & \textbf{Crop Growth Stage} \\
\midrule
Winter & Dec--Feb & Vernalisation, dormancy \\
Spring & Mar--May & Tillering, stem extension, emergence \\
Summer & Jun--Aug & Flowering, grain filling, harvest \\
Autumn & Sep--Nov & Planting, establishment (winter crops) \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Wheat & Random Forest & Winter Sun, Summer Sun, Extreme Summer Rain, Grain Filling Rain \\
...
\subsection{Five Pooling Experiments}

Five experiments were conducted to systematically evaluate cross-country data combination:

\begin{enumerate}
    \item \textbf{Descriptive comparison:} Yield distributions and weather patterns across UK, France, and Germany were compared to identify baseline differences.
    \item \textbf{Direct transfer test:} Models trained on one country were applied to predict another's yields, with all pairs producing negative \rsq{} values, confirming that na\"{\i}ve transfer fails.
    \item \textbf{Country-only baselines:} LOOCV on each country separately established per-country performance benchmarks.
    \item \textbf{Pooled models:} Data from all countries were combined, with and without binary country indicator features ($\text{Is\_France}$, $\text{Is\_Germany}$), using both crop-specific algorithms and a uniform Ridge regression with all 47~features.
    \item \textbf{Feature importance comparison:} Which weather features drive yield variation in each country, revealing both shared sensitivities and country-specific patterns.
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{france_uk_yield_comparison.png}
\caption{Yield comparison between UK and France across all five crops. France generally has lower and more variable yields, with different regional patterns reflecting the continental gradient from Nord (maritime) to Sud (Mediterranean).}
...
\caption{Weather variable comparison between UK and France. France has higher temperatures, lower rainfall, and more sunshine---the climate baseline differences that must be addressed for meaningful data pooling.}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Crop} & \textbf{UK-only} & \textbf{UK Ridge47} & \textbf{Pooled+Country (original)} & \textbf{Pooled+Country (Ridge47)} \\
\midrule
...
Extended the mixed-effects model to allow country-specific weather response slopes for five key variables (Summer Tmax, Spring Rain, Summer Rain, Grain Filling Temp, Summer GDD):
\begin{equation}
    Y_{ij} = \mathbf{x}_{ij}^T \boldsymbol{\beta} + u_{0j} + \sum_{k \in K} u_{kj} x_{kij} + \varepsilon_{ij}
\end{equation}
This was the best method for Winter Barley (\rsq{} = 0.698) and OSR (\rsq{} = 0.613), as these crops have country-specific weather sensitivities that benefit from varying slopes. Feature selection required limiting to $\leq 10$ decorrelated features (greedy selection removing $|r| > 0.85$ pairs) for numerical stability.

\subsection{Method 4: Bias Correction}

Country-specific yield biases were estimated and removed before training: for each country $j$, the mean yield residual $\bar{e}_j$ from a baseline model was subtracted. This achieved the best \textbf{direct transfer} result of \rsq{} = $-3.026$---still negative, but a massive improvement from $-32$ without any correction.

\subsection{Method 5: CORAL Domain Adaptation}

CORAL (CORrelation ALignment) aligns the second-order statistics (covariance) of source and target feature distributions. The source features are transformed by:
\begin{equation}
    \hat{\mathbf{X}}_S = \mathbf{X}_S \mathbf{C}_S^{-1/2} \mathbf{C}_T^{1/2}
\end{equation}
where $\mathbf{C}_S$ and $\mathbf{C}_T$ are source and target covariance matrices. This achieved pooled \rsq{} = 0.488 but was unstable with small country samples.
...
The key conclusion from Phase~2 was that classical domain adaptation methods could improve pooled prediction but could not achieve positive direct transfer. The fundamental barrier was different yield levels and climate baselines between countries---problems that required a different approach.
...
The detrended anomaly transfer methodology was developed to overcome the two barriers identified in Phase~2: (1)~different absolute yield levels between countries, and (2)~different climate baselines. The core insight is to \textbf{transfer weather response shapes rather than absolute predictions}.
...
Ridge regression was used for all crops:
\begin{equation}
...
where $\mathbf{Z}$ is the matrix of standardised weather z-scores and $\alpha$ was tuned per crop ($\alpha \in \{1, 10, 100\}$) via LOOCV.
...
This decomposition is the key: the \textbf{yield trend} (technology, varieties, management) comes from UK's own data (just a simple linear fit requiring minimal data), while the \textbf{yield anomaly} (weather response) comes from the pooled cross-country model with far more training data.

\subsection{Rescaled Direct Transfer}

Beyond pooled prediction, we tested whether a model trained \emph{exclusively} on French anomaly data could predict UK yield anomalies---true cross-country transfer. The France-trained model captures the correct \emph{direction} of weather effects but not necessarily the correct \emph{magnitude} (France's yield variability differs from the UK's). A simple linear calibration was applied via LOOCV on UK data:

\begin{equation}
    \hat{Y}_{\text{anomaly,UK}}^{\text{calibrated}} = a \cdot \hat{Y}_{\text{anomaly,UK}}^{\text{raw}} + b
\end{equation}

where $a$ (scale) and $b$ (shift) are estimated from held-out UK observations. Crucially, this only uses UK data for two calibration parameters, \emph{not} for learning weather--yield relationships. The weather response shapes come entirely from France.
...
\begin{enumerate}
    \item \textbf{UK-only models:} SHAP values computed for the crop-specific algorithms (RF for Wheat/Winter Barley/Oats, Ridge for Spring Barley, SVR for OSR) to identify which weather features drive each crop's predictions.
...
\begin{table}[H]
\centering
\caption{UK-only model performance on test set (2020--2024)}
\label{tab:uk_only_results}
\begin{tabular}{llccl}
\toprule
\textbf{Crop} & \textbf{Algorithm} & \rsq{} & \textbf{RMSE (\tha{})} & \textbf{Interpretation} \\
\midrule
Wheat & Random Forest & 0.50 & 0.60 & Good---50\% variance explained \\
Winter Barley & Random Forest & 0.45 & 0.48 & Good---strong weather signal \\
Spring Barley & Ridge & 0.22 & 0.71 & Moderate---high variability \\
Oats & Random Forest & 0.20 & 0.61 & Moderate---hardy crop \\
OSR & SVR (linear) & 0.15 & 0.52 & Low---pest-dominated \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{High winter frost correlates with better yields}: Counter-intuitively, cold winters improve yields across all crops ($+0.15$ to $+0.35$~\tha{} for high vs.\ low frost years), likely through vernalisation requirements and reduced pest/disease pressure \citep{zheng2015}.
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Pooled anomaly transfer results (LOOCV \rsq{} on UK observations). The improvement column shows the absolute \rsq{} increase over UK-only LOOCV baselines.}
...
The raw correlation column is particularly informative: even without any calibration, the France-trained model's predictions correlate positively with actual UK anomalies ($r = 0.41$--$0.82$). This confirms that the \emph{directions} of weather effects are shared between countries---a hotter-than-normal summer hurts wheat yields in both France and the UK. The linear calibration (just 2~parameters) rescues the predictions from catastrophic negative \rsq{} to positive values, by correcting the magnitude mismatch.

Linear calibration consistently outperformed variance rescaling, suggesting that there is both a scale difference (French yield anomalies have different variance than UK anomalies) and a systematic bias in the transferred predictions.
...
\begin{table}[H]
\centering
\caption{Production model performance in the Flask application (LOOCV \rsq{}). These models use pooled France+UK anomaly data with Ridge regression and all 45 weather features.}
...
\begin{enumerate}
    ...
    \item \textbf{For climate adaptation:} The transferability of weather response curves, combined with \citet{ceglar2019}'s finding that agro-climate zones are migrating northward, suggests that current French weather--yield patterns can inform UK agricultural planning for a warming climate.
\end{enumerate}

\section{Limitations}

\begin{enumerate}
    \item \textbf{Small absolute sample sizes:} Even after pooling, the largest dataset (France+UK) has approximately 112~observations per crop. This limits the complexity of learnable patterns and makes LOOCV results sensitive to individual outlier years.
    ...
    \item \textbf{Missing non-weather factors:} Pest pressure (particularly for OSR), soil quality, and management practices are not modelled. OSR's low \rsq{} (0.443) likely reflects the dominance of cabbage stem flea beetle impacts over weather effects.
...
Corcoran, E., Andersson, E. and Gathorne-Hardy, A. (2023).
\newblock UK crop yield data bottlenecks and opportunities.
\newblock Proceedings paper.
...
\newblock Leave-Two-Out cross-validation for small sample sizes.
\newblock \emph{Environmental Research Letters}.

\bibitem[Evans et~al.(2025)]{evans2025}
Evans, M.R. et~al. (2025).
\newblock Reconsidering space-for-time substitution in ecology and evolution.
\newblock \emph{Nature Climate Change}.
...
\bibitem[Paudel et~al.(2022)]{paudel2022}
Paudel, D., Boogaard, H., de~Wit, A., Janssen, S. et~al. (2022).
...
Riedesel, L. et~al. (2024).
\newblock Site-specific heat and drought impacts on German wheat.
\newblock \emph{Environmental Research Letters}, 19, 034024.

\bibitem[Ronchetti et~al.(2024)]{ronchetti2024}
Ronchetti, G. et~al. (2024).
\newblock Harmonized EU subnational crop statistics.
...
Zhao, C., Liu, B., Piao, S. et~al. (2019).
\newblock Temperature increase reduces global yields of major crops.
\newblock \emph{Nature Climate Change}, 7, 443--449.

\bibitem[Zheng et~al.(2015)]{zheng2015}
Zheng, B., Chenu, K., Dreccer, M.F. and Chapman, S.C. (2015).
\newblock Breeding for the future: what are the potential impacts of future frost and heat events on sowing and flowering time requirements for Australian bread wheat varieties?
...
\newblock \emph{Earth's Future}, 9(4).
